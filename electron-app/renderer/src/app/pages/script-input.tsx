...
export default function ScriptInput() {
  ...
  const handleAnalyze = async () => {
    ...
    if (!isConfigured) {
      alert("설정 페이지에서 API 키를 먼저 설정해주세요.");
      navigate("/settings");
      return;
    }
    ...
    try {
      ...
      const analyzedSegments = await analyzeScriptWithClaude(
        scriptText,
      ...
      // Convert to prompts
      const segmentsWithPrompts = convertToPrompts(
        analyzedSegments,
        styleId || "personification",
        globalContext || ""
      );

      setSegments(segmentsWithPrompts);

      // Go to image review/settings
      navigate("/detailed-settings");
    } catch (err) {
      console.error(err);
      alert("대본 분석에 실패했습니다. API 키를 확인해주세요.");
    } finally {
